...
针对 generativelanguage.googleapis.com 域名
替代 genca.sh 和 gencrt.sh 的功能

使用方法:
1. 运行脚本一键生成CA证书和服务器证书: python generate_google_certs.py
...
"""
    ...
    # 创建 generativelanguage.googleapis.com.cnf
    google_cnf = """
[ v3_req ]
basicConstraints = CA:FALSE
keyUsage = nonRepudiation, digitalSignature, keyEncipherment
...
[alt_names]
DNS.1 = generativelanguage.googleapis.com
DNS.2 = *.googleapis.com
DNS.3 = googleapis.com
"""

    # 创建 generativelanguage.googleapis.com.subj
    google_subj = "/C=US/ST=California/L=Mountain View/O=Google LLC/OU=Google Cloud/CN=generativelanguage.googleapis.com"
    ...
    files_to_create = {
        ...
        os.path.join(ca_dir, "generativelanguage.googleapis.com.cnf"): google_cnf,
        os.path.join(ca_dir, "generativelanguage.googleapis.com.subj"): google_subj
    }
...
def generate_ca_cert():
    ...
    organization = "Google Local CA"
    org_unit = "Certificate Authority"
    common_name = "Google Local CA"
    email = ""
...
def generate_server_cert(ca_key_path, ca_crt_path, domain="generativelanguage.googleapis.com"):
    """生成服务器证书"""
...
def main():
    """主函数"""
    # 解析命令行参数
    parser = argparse.ArgumentParser(description='一键生成CA证书和Google API服务器证书')
    parser.add_argument('--domain', default='generativelanguage.googleapis.com', help='服务器证书的域名 (默认: generativelanguage.googleapis.com)')
    args = parser.parse_args()
    
    print("\n" + "="*70)
    print("Google API 证书生成工具 - 一键生成CA证书和服务器证书")
    print("="*70 + "\n")
    ...
    print("2. 修改hosts文件，将generativelanguage.googleapis.com指向127.0.0.1")
    print("3. 配置代理服务器使用生成的证书")
    print("4. 确保代理服务器监听443端口并使用SSL/TLS")
    print("="*70 + "\n")
