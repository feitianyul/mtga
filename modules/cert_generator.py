...
def generate_ca_cert(resource_manager, log_func=print):
    ...
    with open(openssl_cnf_path, 'r', encoding='utf-8') as f:
        openssl_cnf = f.read()
    with open(v3_ca_cnf_path, 'r', encoding='utf-8') as f:
        v3_ca_cnf = f.read()
    ...
    success, _ = run_openssl_command(
        [
            resource_manager.openssl_path, "req", "-new", "-x509", "-extensions", "v3_ca",
...
def generate_server_cert(resource_manager, domain="api.openai.com", log_func=print):
    """生成服务器证书"""
    ...
    # 读取配置文件
    with open(resource_manager.get_config_file("openssl.cnf"), 'r', encoding='utf-8') as f:
        openssl_cnf = f.read()
    with open(resource_manager.get_config_file("v3_req.cnf"), 'r', encoding='utf-8') as f:
        v3_req_cnf = f.read()
    with open(resource_manager.get_config_file(f"{domain}.cnf"), 'r', encoding='utf-8') as f:
        domain_cnf = f.read()
    ...
    # 读取主题信息
    with open(resource_manager.get_config_file(f"{domain}.subj"), 'r', encoding='utf-8') as f:
    ...
    success, _ = run_openssl_command(
        [
            resource_manager.openssl_path, "pkcs8", "-topk8", "-nocrypt",
    ...
    success, _ = run_openssl_command(
        [
            resource_manager.openssl_path, "req", "-reqexts", "v3_req", "-sha256", "-new",
...
def generate_certificates(domain="api.openai.com", log_func=print):
    ...
    # 检查 OpenSSL 是否可用
    try:
        result = subprocess.run([resource_manager.openssl_path, "version"], 
