  ...
  <div ref="containerRef" class="form-control inline-block relative">
    ...
    <div 
      ...
      :class="[
        ...
        isOpen ? 'border-primary ring-4 ring-primary/20 bg-white' : 'hover:border-primary/40 hover:bg-white',
    ...
    >
      ...
      >
        <div v-if="normalizedOptions.length === 0" class="px-4 py-3 text-center text-slate-400 text-xs">
          暂无选项
        </div>
        <ul v-else class="max-h-60 overflow-y-auto custom-scrollbar">
          <li v-for="opt in normalizedOptions" :key="opt.value">
            ...
            >
              ...
              <!-- 选中标记 -->
              <svg v-if="modelValue === opt.value" xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-primary" fill="none" viewBox="0 0 24 24" stroke="currentColor">
